...
#
# Usage :
#   curl -fsSL https://raw.githubusercontent.com/FabrichJean/c-sha/main/agent/install.sh | bash
set -euo pipefail
...
if [ -n "${LEDGER_URL:-}" ] && [ -n "${LEDGER_SYNC_API_KEY:-}" ]; then
  ...
  echo "  LEDGER_URL=https://ton-serveur LEDGER_SYNC_API_KEY=ta_cle bash -c \"\$(curl -fsSL https://raw.githubusercontent.com/$REPO/main/agent/install.sh)\""
  echo
